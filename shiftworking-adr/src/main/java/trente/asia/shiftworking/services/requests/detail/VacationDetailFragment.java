...
import trente.asia.shiftworking.services.requests.adapter.VacationApproveHistoryAdapter;
...
public class VacationDetailFragment extends AbstractSwFragment{
	...
	private void setWorkOffer(VacationRequestModel vacationRequest){
		try{
			...
			Gson gson = new Gson();
			CAObjectSerializeUtil.deserializeObject(lnrContent, new JSONObject(gson.toJson(vacationRequest)));
		}catch(JSONException e){
			e.printStackTrace();
		}

		((TextView)getView().findViewById(R.id.txt_fragment_offer_detail_offer_user)).setText(vacationRequest.userName);
		((TextView)getView().findViewById(R.id.txt_fragment_offer_detail_offer_type)).setText(vacationRequest.vacationName);
		((TextView)getView().findViewById(R.id.txt_fragment_offer_detail_start_date)).setText(vacationRequest.startDateString);
		((TextView)getView().findViewById(R.id.txt_fragment_offer_detail_note)).setText(vacationRequest.note);
	...
	private void buildWorkOfferApproveHistory(){
		if(vacationRequest.userId.equals(myself.key)){
			...
			VacationApproveHistoryAdapter adapter = new VacationApproveHistoryAdapter(activity, vacationRequest.listHistories);
	...
	private void sendApproveResult(String approveResult){
		JSONObject jsonObject = new JSONObject();
		try{
			jsonObject.put("key", vacationRequest.key);
