...
public class OvertimeDetailFragment extends AbstractSwFragment{
	...
	private void setOtRequestModel(OtRequestModel otRequest){
		try{
			LinearLayout lnrContent = (LinearLayout)getView().findViewById(R.id.lnr_id_content);
			Gson gson = new Gson();
			CAObjectSerializeUtil.deserializeObject(lnrContent, new JSONObject(gson.toJson(otRequest)));
		}catch(JSONException e){
			e.printStackTrace();
		}

		((TextView)getView().findViewById(R.id.txt_fragment_overtime_detail_offer_user)).setText(otRequest.userName);
		((TextView)getView().findViewById(R.id.txt_fragment_overtime_detail_offer_type)).setText(typeListMap.get(this.otRequest.overtimeType));
		((TextView)getView().findViewById(R.id.txt_fragment_overtime_detail_start_date)).setText(otRequest.startDateString);
		((TextView)getView().findViewById(R.id.txt_fragment_overtime_detail_start_time)).setText(otRequest.startTimeString + "-" + otRequest.endTimeString);
		((TextView)getView().findViewById(R.id.txt_fragment_overtime_detail_reason)).setText(otRequest.note);
	}
	...
	private void judgeAprovePermission(){
		...
		if(otRequest.listHistories != null){
			history = otRequest.listHistories.get(otRequest.listHistories.size() - 1);
	...
	private void gotoWorkOfferEditFragment(){
		...
		fragment.setActiveOfferId(otRequest.key);
	...
	private void sendApproveResult(String approveResult){
		JSONObject jsonObject = new JSONObject();
		try{
			jsonObject.put("key", otRequest.key);
