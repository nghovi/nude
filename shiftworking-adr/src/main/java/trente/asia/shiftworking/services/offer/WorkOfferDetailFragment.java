...
public class WorkOfferDetailFragment extends AbstractSwFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (mRootView == null) {
            mRootView = inflater.inflate(R.layout.fragment_offer_detail,
                    container, false);
        }
        return mRootView;
    }
    ...
    private void buildOfferInfo() {
        ...
        ((TextView) getView().findViewById(R.id
                .txt_fragment_offer_detail_offer_type)).setText(offerType);

        ((TextView) getView().findViewById(R.id
                .txt_fragment_offer_detail_start_date)).setText(offer
                .startDateString);

        ((TextView) getView().findViewById(R.id
                .txt_fragment_offer_detail_end_date)).setText(offer
                .endDateString);

        if (CCStringUtil.isEmpty(offer.startTimeString)) {
            getView().findViewById(R.id.lnr_start_time).setVisibility(View
                    .INVISIBLE);
        } else {
            ((TextView) getView().findViewById(R.id
                    .txt_fragment_offer_detail_start_time)).setText(offer
                    .startTimeString);
        }

        if (CCStringUtil.isEmpty(offer.endTimeString)) {
            getView().findViewById(R.id.lnr_end_time).setVisibility(View
                    .INVISIBLE);
        } else {
            ((TextView) getView().findViewById(R.id
                    .txt_fragment_offer_detail_end_time)).setText(offer
                    .endTimeString);
        }

        TextView txtNote = (TextView) getView().findViewById(R.id
                .txt_fragment_offer_detail_note);
        txtNote.setText(offer.note);
    }

    private void buildActionButtons() {
        Button btnReject = (Button) getView().findViewById(R.id
                .btn_fragment_offer_detail_reject);

        Button btnApprove = (Button) getView().findViewById(R.id
                .btn_fragment_offer_detail_approve);

        if (WorkOffer.OFFER_STATUS_OFFER.equals(offer.offerStatus) &&
                WorkOffer.OFFER_PERMISSION_APPROVEABLE.equals(offer
                        .permission)) {
        ...
        Button btnDelete = (Button) getView().findViewById(R.id
                .btn_fragment_offer_detail_delete);
        if (WorkOffer.OFFER_STATUS_OFFER.equals(offer.offerStatus) && offer
                .userId.equals(myself.loginUserId)) {
    ...
    private void buildWorkOfferApproveHistory() {
        ChiaseListViewNoScroll lstApproveHistory = (ChiaseListViewNoScroll)
                getView().findViewById(R.id
                        .lst_fragment_offer_detail_approve_history);
        ApproveHistoryAdapter adapter = new ApproveHistoryAdapter(activity,
                offer.listHistories);
    ...
    protected void buildHeaderWithBackBtn(String title) {
        super.initHeader(R.drawable.wf_back_white, title, R.drawable
                .ic_action_remove);
        btnEdit = (ImageView) getView().findViewById(R.id
    ...
    private void onClickBtnDelete() {
        ...
        dlgConfirmDelete.setDialogTitleButton(getString(R.string
                .fragment_offer_edit_confirm_delete_msg), getString(android.R
                .string.ok), getString(android.R.string.cancel), new View
                .OnClickListener() {
    ...
    protected void successLoad(JSONObject response, String url) {
        offer = CCJsonUtil.convertToModel(response.optString("offer"),
                WorkOffer.class);
        offerTypesMaster = buildOfferTypeMaster(activity, response);
        offerStatusMaster = buildOfferStatusMaster(activity, response);

        groupInfo = CCJsonUtil.convertToModel(response.optString
                ("groupJoinMap"), Map.class);
        if (offer == null) {
            ((MainActivity) activity).isInitData = true;
            onClickBackBtn();
        } else {
            targetUserModels = CCJsonUtil.convertToModel(response.optString
                    ("targetUserModel"), Map.class);
            buildWorkOfferDetail();
        }
    }

    public void setOfferTypeStatusMaster(Map<String, String>
                                                 offerTypesMaster, Map<String,
            String> offerStatusMaster) {
    ...
    public static class ApproveStatus {
        ...
        public ApproveStatus(String status1, String status2, int userType,
                             int approveGroupType) {
        ...
        public int hashCode() {
            String result = "1";
            result += this.status1 + this.status2 + this.userType + this
                    .approveGroupType;
        ...
        public boolean equals(final Object obj) {
            ...
            return this.status1.equals(other.status1) && this.status2.equals
                    (other.status2) && this.userType == other.userType &&
                    this.approveGroupType == other.approveGroupType;
        }
    }
