...
public class KpiCalendarAdapter extends BaseAdapter{
	...
	private final List<ActualPlan>	actualPlanList;
	...
	public KpiCalendarAdapter(Context c, GregorianCalendar monthCalendar, List<ActualPlan> actualPlanList){
		this.actualPlanList = actualPlanList;
		KpiCalendarAdapter.dayString = new ArrayList<Calendar>();
	...
	public View getView(int position, View convertView, ViewGroup parent){
		...
		if(((gridvalue > 1) && (position < firstDay)) || ((gridvalue < 7) && (position > 28))){
			...
			ActualPlan actualPlan = ActualPlanAddFragment.getActualPlanByDay(dayString.get(position), actualPlanList);
			if(DRUtil.getDateString(Calendar.getInstance().getTime(), DRConst.DATE_FORMAT_YYYY_MM_DD).equals(DRUtil.getDateString(actualPlan.date, DRConst.DATE_FORMAT_YYYY_MM_DD_HH_MM_SS, DRConst.DATE_FORMAT_YYYY_MM_DD))){
