...
public class ScheduleFormFragment extends AbstractClFragment {
    ...
    protected void initView() {
        ...
        txtRoom = (ChiaseTextView) getView().findViewById(R.id
                .txt_fragment_schedule_form_room);
        if (schedule != null) {
            txtRoom.setText(rooms.get(Integer.parseInt(schedule.roomId)).value);
            txtRoom.setValue(schedule.roomId);
        }
        getView().findViewById(R.id.lnr_fragment_schedule_form_room)
                .setOnClickListener(this);
        txtStartDate = (ChiaseTextView) getView().findViewById(R.id
                .fragment_schedule_detail_start_date);
        txtEndDate = (ChiaseTextView) getView().findViewById(R.id
                .fragment_schedule_detail_end_date);
        txtStartTime = (ChiaseTextView) getView().findViewById(R.id
                .fragment_schedule_detail_start_time);
        txtEndTime = (ChiaseTextView) getView().findViewById(R.id
                .fragment_schedule_detail_end_time);

        txtStartDate.setOnClickListener(this);
        txtEndDate.setOnClickListener(this);
        txtStartTime.setOnClickListener(this);
        txtEndTime.setOnClickListener(this);
        horizontalUserListView = (HorizontalUserListView) getView()
                .findViewById(R.id.view_horizontal_user_list);
    ...
    private void buildDatePickerDialogs(ScheduleModel schedule) {
        ...
        if (schedule != null && !CCStringUtil.isEmpty(schedule.key)) {
            starDate = CCDateUtil.makeDateCustom(schedule.startDate,
                    WelfareConst.WL_DATE_TIME_7);
            endDate = CCDateUtil.makeDateCustom(schedule.endDate,
                    WelfareConst.WL_DATE_TIME_7);
            startHour = CCStringUtil.isEmpty(schedule.startTime) ? 0 :
                    CCNumberUtil.toInteger(schedule.startTime.split(":")[0]);
            startMinute = CCStringUtil.isEmpty(schedule.startTime) ? 0 :
                    CCNumberUtil.toInteger(schedule.startTime.split(":")[1]);
            endHour = CCStringUtil.isEmpty(schedule.endTime) ? 0 :
                    CCNumberUtil.toInteger(schedule.endTime.split(":")[0]);
            endMinute = CCStringUtil.isEmpty(schedule.endTime) ? 0 :
                    CCNumberUtil.toInteger(schedule.endTime.split(":")[1]);
        ...
        txtStartDate.setText(CCFormatUtil.formatDateCustom(WelfareConst
                .WL_DATE_TIME_7, starDate));
        txtStartTime.setText(getDisplayNum(startHour) + ":" +
                getDisplayNum(startMinute));
        txtEndDate.setText(CCFormatUtil.formatDateCustom(WelfareConst
                .WL_DATE_TIME_7, endDate));
        txtEndTime.setText(getDisplayNum(startHour) + ":" +
                getDisplayNum(startMinute));

        calendar.setTime(starDate);
        datePickerDialogStart = new DatePickerDialog(activity, new
                DatePickerDialog.OnDateSetListener() {
                    ...
                    public void onDateSet(DatePicker view, int year, int
                            month, int
                                                  dayOfMonth) {
                        String startDateStr = year + "/" + getDisplayNum(month +
                                1) +
                                "/" + getDisplayNum(dayOfMonth);
        ...
        datePickerDialogEnd = new DatePickerDialog(activity, new
                DatePickerDialog.OnDateSetListener() {
                    ...
                    public void onDateSet(DatePicker view, int year, int
                            month, int
                                                  dayOfMonth) {
                        String endDateStr = year + "/" + getDisplayNum(month +
                                1) +
                                "/" + getDisplayNum(dayOfMonth);
        ...
        timePickerDialogEnd = new TimePickerDialog(activity, new
                TimePickerDialog.OnTimeSetListener() {

                    @Override
                    public void onTimeSet(TimePicker view, int hourOfDay, int
                            minute) {
                        txtEndTime.setText(getDisplayNum(hourOfDay) + ":" +
                                getDisplayNum(minute));
                        txtEndTime.setValue(getDisplayNum(hourOfDay) + ":" +
                                getDisplayNum(minute));
                    }
                }, endHour, endMinute, true);

        timePickerDialogStart = new TimePickerDialog(activity, new
                TimePickerDialog.OnTimeSetListener() {

                    @Override
                    public void onTimeSet(TimePicker view, int hourOfDay, int
                            minute) {
                        txtStartTime.setText(getDisplayNum(hourOfDay) + ":" +
                                getDisplayNum(minute));
                        txtStartTime.setValue(getDisplayNum(hourOfDay) + ":" +
                                getDisplayNum(minute));
                    }
    ...
    private void showChooseRoomDialog() {
        if (dlgChooseRoom != null) {
            ...
            dlgChooseRoom = new ChiaseListDialog(getContext(), "Select " +
                    "venue", WelfareFormatUtil.convertList2Map(rooms),
                    txtRoom, null);
            dlgChooseRoom.show();
        }
    }
    ...
    private void onLoadScheduleDetailSuccess(JSONObject response) {
        schedule = CCJsonUtil.convertToModel(response.optString("schedule"),
                ScheduleModel.class);
        rooms = CCJsonUtil.convertToModelList(response.optString("rooms"),
                ApiObjectModel.class);
        calendars = CCJsonUtil.convertToModelList(response.optString
                ("calendarListDetail"), CalendarModel.class);
        List<UserModel> calendarUsers = CCJsonUtil.convertToModelList
                (response.optString("calendarUsers"), UserModel.class);

        List<UserModel> joinUserList = ScheduleDetailFragment
                .getJoinedUserModels(schedule, CCJsonUtil.convertToModelList
                        (response.optString("calendarUsers"), UserModel
                                .class));
        try {
            Gson gson = new Gson();
            CAObjectSerializeUtil.deserializeObject((ViewGroup) getView()
        ...
        horizontalUserListView.inflateWith(joinUserList, calendarUsers, true,
                32, 10);
    ...
    private void sendUpdatedRequest() {
        ...
        try {
            ...
            jsonObject.put("calendarId", calendars.get(0).key);//// TODO:
