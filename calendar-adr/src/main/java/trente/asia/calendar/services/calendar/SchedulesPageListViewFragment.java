...
public abstract class SchedulesPageListViewFragment extends
        SchedulesPageFragment implements CalendarDayView.OnDayClickListener {
    ...
    protected void clearOldData() {
        for (CalendarDayView dayView : calendarDayViews) {
            CalendarDayModel calendarDayModel = getCalendarDayModel(dayView
                    .getDate(), calendarDayModels);
    ...
    public List<CalendarDayModel> buildCalendarDayModels(List<ScheduleModel>
        ...
        for (ScheduleModel scheduleModel : schedules) {
            ...
            if (scheduleModel.isPeriodSchedule()) {
                ...
                Date limit = endDate.compareTo(endDisplayed) <= 0 ? endDate :
                        endDisplayed;
                while (CCDateUtil.compareDate(startDate, limit, false) <= 0) {
                    addCalendarDayModel(startDate, scheduleModel,
                            calendarDayModels);
                    c.setTime(startDate);
        ...
        for (HolidayModel holidayModel : lstHoliday) {
            ...
            CalendarDayModel calendarDayModel = getCalendarDayModel(date,
                    calendarDayModels);
        ...
        for (UserModel userModel : lstBirthdayUser) {
            Date date = WelfareUtil.makeDate(userModel.dateBirth);
            CalendarDayModel calendarDayModel = getCalendarDayModel(date,
                    calendarDayModels);
            if (calendarDayModel == null) {
                calendarDayModel = new CalendarDayModel();
    ...
    public void addCalendarDayModel(Date date, ScheduleModel scheduleModel,
    ...
    public CalendarDayModel getCalendarDayModel(Date date,
        ...
        if (!CCCollectionUtil.isEmpty(calendarDayModels)) {
            for (CalendarDayModel calendarDayModel : calendarDayModels) {
                if (CCFormatUtil.formatDateCustom(WelfareConst
                        .WL_DATE_TIME_7, calendarDayModel.date).equals
                        (CCFormatUtil.formatDateCustom(WelfareConst
                                .WL_DATE_TIME_7, date))) {
